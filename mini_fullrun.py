"""
mini_fullrun.py — Run all 242 pairs through gpt-4.1-mini with a $25 cost kill switch.
...
def main():
    ...
    # Submit all remaining chunks in parallel, then poll together
    if state is None:
    ...
    for chunk_idx, chunk in enumerate(chunks):
        if chunk_idx in completed_chunks:
            continue
        # Check if already submitted but not yet done (resume case)
    ...
    # Poll all pending batches together
    while pending:
